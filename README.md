...
- **Graphite** — small eigenstrain (Ω·c_max ≈ 8 %), linear elasticity is valid,
  ~18 MPa center tension during 1C lithiation, critical radius far above
  commercial particle sizes → **safe**.
- **Silicon** — huge eigenstrain (Ω·c_max ≈ 300 %) and near-step concentration
...
| | Graphite | Silicon |
|---|---|---|
| R, D | 8 µm, 3.9e-14 m²/s | 3 µm, 1.0e-16 m²/s |
| E, ν, σ_Y | 15 GPa, 0.30, — (elastic) | 90 GPa, 0.22, 1.5 GPa |
| Eigenstrain Ω·c_max | 0.084 (~8 %) | 3.00 (~300 %) |
| Raw elastic peak hoop | 18.2 MPa (center) | 113 GPa > E — small-strain breakdown |
| Corrected (Tresca) | n/a | r_p/R = 0.52; center 2.42 GPa; surface −1.49 GPa |
...
Sensitivity of the silicon R_crit: E, Ω, K_IC at ±20 % → 296–366 nm; σ_Y →
324–347 nm; flaw depth a = 0.05–0.2R → 280–405 nm. The result is robust — the
correction, not parameter tuning, brings R_crit into the experimental window.
...
- **Plastic correction**: where the trial elastic field violates the Tresca
  condition at the surface, a plastic shell (σθ − σr = ±σ_Y) is assembled on a
  brentq-solved interface radius r_p around an elastic core that carries the
...
- **Fracture**: Griffith surface-flaw criterion σ_f = K_IC / √(aπR) with flaw
  depth a = 0.1R; R_crit solves peak σθ(R) = σ_f(R) by bracketing + brentq.
...
- Constant material properties: D, E, ν, Ω, K_IC and the σ_Y plateau are
  ...
  effect of these simplifications on the silicon R_crit (296–366 nm).
- Constant-flux Fickian diffusion allows c_surf > c_max (no saturation); it is
